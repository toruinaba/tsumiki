...
- [開発者ガイド：カード・計算ロジックの追加](#開発者ガイドカード計算ロジックの追加)
  ...
  - [dynamicInputGroups — 動的ペア行](#5-dynamicinputgroups--動的ペア行)
  - [カードの登録](#6-カードの登録)
- [ユニットシステム](#ユニットシステム)
- [データモデル](#データモデル)
- [ファイル構成](#ファイル構成)
...
```bash
# 公開 URL: https://toruinaba.github.io/tsumiki/
# （手動デプロイは不要）
```
...
| | 等分布荷重 (uniform) | 集中荷重 (point) | 集中モーメント (moment) |
|---|---|---|---|
| **単純梁** (simple) | `simple::uniform` | `simple::point` | `simple::moment` |
| **片持ち梁** (cantilever) | `cantilever::uniform` | `cantilever::point` | `cantilever::moment` |
| **両端固定** (fixed_fixed) | `fixed_fixed::uniform` | `fixed_fixed::point` | `fixed_fixed::moment` |
| **一端固定一端ピン** (fixed_pinned) | `fixed_pinned::uniform` | `fixed_pinned::point` | `fixed_pinned::moment` |

出力: `Mmax`, `Qmax`, `delta_max`, `diagramModel`（Diagram・Deflection カードへの参照用モデル）

### VERIFY — 断面照査

任意の実測値と許容値を受け取り、応力度比を計算するシンプルな照査カードです。

| 入力 | 説明 |
|------|------|
| `value` | 実測値（任意の値 — 応力度・力など単位は問わない）|
| `allowable` | 許容値 |

出力: `ratio`（= \|value\| / allowable）, `margin`（= 1 − ratio）, `isOk`（1=OK / 0=NG）

### CUSTOM_MAP — カスタム数式（1 入力→1 出力）

変数を自由に定義し、mathjs の数式で計算します。変数は他カードの出力に参照リンクできます。

### CUSTOM_COMBINE — カスタム合成（複数入力→複数出力）

複数の入力変数を組み合わせた数式を記述できます。各出力フィールドに独立した数式を設定します。
...
距離行は UI から動的に追加・削除できます（後述の `dynamicInputGroups` 機能）。
...
createStrategyDefinition({
    ...
    strategies: [
        // id は各軸の value を '::' で結合したもの
        { id: 'simple::uniform',     label: 'Simple / Uniform',     inputConfig: {...}, calculate: (...) => {...} },
        { id: 'simple::point',       label: 'Simple / Point',       inputConfig: {...}, calculate: (...) => {...} },
        { id: 'cantilever::uniform', label: 'Cantilever / Uniform', inputConfig: {...}, calculate: (...) => {...} },
        { id: 'cantilever::point',   label: 'Cantilever / Point',   inputConfig: {...}, calculate: (...) => {...} },
    ],
    // ...
});
```

ストラテジーの `id` は各軸の選択値を `::` で結合した文字列になります（例: `'simple::uniform'`）。
...
export const CoupleCardDef = createCardDefinition({
    ...
    // ── dynamicInputGroups の設定（配列で複数グループ定義可能）──────────
    dynamicInputGroups: [{
...
**`DynamicInputGroupConfig` のフィールド一覧**（`dynamicInputGroups` 配列の各要素）
...
**編集が必要なのは 1 ファイルのみ**です。

`src/components/cards/MyCard.tsx` を作成し、末尾で `registry.register()` を呼ぶだけで登録が完了します。
...
registry.register(MyCardDef); // ← これだけで登録完了
```

`src/lib/registry/index.ts` が `import.meta.glob('../../components/cards/*.tsx', { eager: true })` で
`cards/` 以下の全ファイルを自動ロードするため、手動インポートは不要です。
サイドバーへの表示は `CardDefinition.sidebar` で宣言します（AppLayout の編集不要）。
`CardType` は `string` 型なので `src/types/index.ts` の編集も不要です。
...
```typescript
// src/types/index.ts
export type CardType = string; // 文字列型（拡張時に型定義編集不要）
...
export interface CardInput {
    ...
    ref?: {
        cardId: string;
        outputKey: string;
        expression?: string; // 数式変換（例: 'v/2'）
        refType?: 'output' | 'input'; // 出力参照 or 入力参照
        inputKey?: string;  // refType='input' のとき参照先の入力キー
    };
}
...
export interface Card {
    ...
    memo?: string;       // ユーザーメモ
    resolvedInputs?: Record<string, number>; // 最終計算後の解決済み入力値（シリアライズ対象外）
}
```
...
> **注**: `cards/` 以下のカードファイルは代表例のみ記載しています。実際には多数のカードタイプが実装されています。
...
│   │   ├── Custom.tsx                # カスタム数式カード定義（CUSTOM_MAP / CUSTOM_COMBINE）
│   │   ├── Material.tsx              # 材料カード定義
│   │   ├── Section.tsx               # 断面カード定義
│   │   └── Verify.tsx                # 照査カード定義
│   │   └── …                         # その他カードファイル（BeamMulti, Diagram, Deflection, Column 等）
│   ├── common/
